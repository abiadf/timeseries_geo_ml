...
class Autoencoder(nn.Module):
    ...
    def __init__(self, layer_dims: list[int], dropout_prob: float = 0.05, projection_dim: int = 16):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, return_latent: bool = False, return_projection: bool = False):
        """Forward pass through encoder and decoder.
            - x (torch.Tensor): Input tensor of shape (batch_size, input_dim)
            - Returns latent code z and reconstruction x̂"""
        x     = x.to(device)
        z     = self.encoder(x)
...
class TrainAutoencoder:
    ...
    def _train_epoch(self, device: torch.device, autoencoder: Autoencoder, train_loader: DataLoader, optimizer: optim.Optimizer) -> float:
        ...
        for data in train_loader:
            ...
            optimizer.zero_grad()
            x_reconstructed, _ = autoencoder(x_input)
