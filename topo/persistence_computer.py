...
MIN, MAX, GMIN, GMAX = 1, -1, 2, -2

# ========= 1) Base + streaming case ========

def find_extrema_in_timeseries(x, device) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    gmin_val = x.min()
    gmax_val = x.max()
    gmin_idx = torch.nonzero(x == gmin_val).flatten()
    gmax_idx = torch.nonzero(x == gmax_val).flatten()
    ...
    for idx, neighbor in [(0, 1), (len(x) - 1, len(x) - 2)]:
        val = x[idx]
        if val == gmax_val:
            pass  # discard
        elif val < x[neighbor]:
    ...
    left_was_gmax  = x[0]   == gmax_val
    right_was_gmax = x[-1]  == gmax_val
    boundary_stole_gmax = (left_was_gmax or right_was_gmax) and not torch.isin(max_idx, gmax_idx).any()
    if boundary_stole_gmax:
        interior_max_val = x[1:-1].max()
        promoted_idx     = torch.nonzero(x[1:-1] == interior_max_val).flatten() + 1
...
def _run_fast_sweep_loop(sweep_line_schedule: np.ndarray, keypoint_types: np.ndarray, keypoint_idx_array: np.ndarray, timeseries_values: np.ndarray, num_keypoints: int):
    """Standard full-batch sweep engine"""
...
class OnlineSublevelPersistence:
    ...
    def __init__(self, initial_x: torch.Tensor, initial_keypoint_idx: torch.Tensor, initial_keypoint_types: torch.Tensor, max_capacity: int = 10_000_000):
        ...
        self.current_values_len = len(initial_x)
        ...
        _run_streaming_sweep_loop(
            initial_ranks, self.keypoint_types, self.keypoint_idx_array,
    ...
    def append_stream_data(self, new_x_chunk: torch.Tensor, stream_keypoint_idx: torch.Tensor, stream_keypoint_types: torch.Tensor):
            ...
            new_pairs = _run_streaming_sweep_loop(
                ranks_to_process, self.keypoint_types, self.keypoint_idx_array,
                self.history_values, self.basin_membership_ids, self.submerged_components,
                self.num_keypoints)
            return new_pairs


# ===== 2) just BATCHING (no streaming) ======
...
def _run_batch_sweep_loop(
    sweep_line_schedules, keypoint_types, keypoint_idx_arrays, 
    timeseries_matrix, actual_lengths, 
    basin_membership_ids, submerged_components):  # <-- Pre-allocated blocks passed here
    """Processes N independent time series concurrently across CPU cores with ZERO internal allocations."""
    N = timeseries_matrix.shape[0]
    max_keypoints = sweep_line_schedules.shape[1]
    
    batch_pairs_storage = np.full((N, max_keypoints, 2), -1, dtype=np.int64)
...
def compute_batch_sublevel_persistence(timeseries_matrix: torch.Tensor, idx_list: list, types_list: list):
    ...
    padded_idx   = np.zeros((N, max_len), dtype=np.int64)
    padded_types = np.zeros((N, max_len), dtype=np.int64)
    padded_sweep = np.zeros((N, max_len), dtype=np.int64)

    # 1. Allocate large shared 2D tracking blocks out here in Python space once
    basin_membership_ids = np.zeros((N, max_len), dtype=np.int64)
    submerged_components = np.zeros((N, max_len), dtype=np.bool_)
...
def compute_sublevel_persistence(timeseries: torch.Tensor, keypoint_idx, keypoint_types):
    """Smart router that dynamically selects the fastest architecture."""
    if timeseries.ndim == 1 or (timeseries.ndim == 2 and timeseries.shape[0] == 1):
        ...
        # Route multi-signal tensors to the multi-core parallel engine
        return compute_batch_sublevel_persistence(timeseries, keypoint_idx, keypoint_types)

@njit(fastmath=True)
def _numba_find_root_batch_online(basin_membership_ids, row, rank):
    cursor = rank
    while cursor != basin_membership_ids[row, cursor]:
        basin_membership_ids[row, cursor] = basin_membership_ids[row, basin_membership_ids[row, cursor]]
        cursor = basin_membership_ids[row, cursor]
...
def _run_batch_streaming_sweep_loop(
    start_ranks, end_ranks,          # 1D arrays tracking the new slice per row
    keypoint_types, keypoint_idx_matrix,
    timeseries_matrix, basin_membership_ids, submerged_components):
    ...
    batch_pairs_storage = np.full((N, max_kp_per_row, 2), -1, dtype=np.int64)
    pair_counts = np.zeros(N, dtype=np.int64)
    ...
    for i in prange(N):
        ...
        for idx in sorted_ranks_idx:
            ...
            if keypoint_type == MIN or keypoint_type == GMIN:
                ...
                left_root  = _numba_find_root_batch_online(basin_membership_ids, i, left_rank) if left_rank >= 0 else -1
                right_root = _numba_find_root_batch_online(basin_membership_ids, i, right_rank) if right_rank < kp_end else -1
...
class BatchOnlineSublevelPersistence:
    ...
    def __init__(self, initial_matrix: torch.Tensor, idx_list: list, types_list: list, max_capacity: int = 10_000_000):
        self.N = initial_matrix.shape[0]
        self.device = initial_matrix.device
        
        # Pre-allocate large continuous 2D matrices
        self.history_values = np.zeros((self.N, max_capacity), dtype=np.float32)
        self.keypoint_types = np.zeros((self.N, max_capacity), dtype=np.int64)
        self.keypoint_idx_matrix = np.zeros((self.N, max_capacity), dtype=np.int64)
        
        # Tracking states
        self.basin_membership_ids = np.zeros((self.N, max_capacity), dtype=np.int64)
        for i in range(self.N):
            self.basin_membership_ids[i] = np.arange(max_capacity)
            
        self.submerged_components = np.zeros((self.N, max_capacity), dtype=np.bool_)
        
        # Track individual filled boundary sizes per channel
        self.current_values_lens = np.full(self.N, initial_matrix.shape[1], dtype=np.int64)
        ...
        start_ranks = np.zeros(self.N, dtype=np.int64)
        _run_batch_streaming_sweep_loop(
            start_ranks, self.num_keypoints,
            self.keypoint_types, self.keypoint_idx_matrix,
            self.history_values, self.basin_membership_ids, self.submerged_components)
    ...
    def append_batch_stream(self, new_chunks_matrix: torch.Tensor, stream_idx_list: list, stream_types_list: list):
        ...
        end_ranks = np.zeros(self.N, dtype=np.int64)
        ...
        return _run_batch_streaming_sweep_loop(
            start_ranks, end_ranks,
            self.keypoint_types, self.keypoint_idx_matrix,
            self.history_values, self.basin_membership_ids, self.submerged_components)
