...
from src.timevae_torch.src.vae.vae_base import BaseVariationalAutoencoder, Sampling
...
class ConvDecoder(nn.Module):
    def __init__(self, seq_len, feat_dim, hidden_layer_sizes, latent_dim, encoder_last_dense_dim):
        ...
        # # store encoder size for use in forward
        # self.encoder_last_dense_dim = encoder_last_dense_dim
        # assert self.encoder_last_dense_dim % self.hidden_layer_sizes[-1] == 0, (
        #     "encoder_last_dense_dim must be divisible by last hidden channel count")

        # with torch.no_grad():
        #     dummy = torch.randn(1, latent_dim)
        #     x = F.relu(self.dense(dummy))
        #     x = x.view(1,
        #             self.hidden_layer_sizes[-1],
        #             self.encoder_last_dense_dim // self.hidden_layer_sizes[-1])
        #     for deconv in self.deconv_layers:
        #         x = F.relu(deconv(x))
        #     L_final = x.shape[2]
        # print("L_final inferred:", L_final)
        # if self.feat_dim * L_final > 1_000_000:
        #     raise RuntimeError(f"final dense too big: feat_dim*L_final={self.feat_dim*L_final}")
        # self.final_dense = nn.Linear(self.feat_dim * L_final, self.seq_len * self.feat_dim)


        # debug print — helps identify which layer has insane size
        total = 0
        print("encoder_last_dense_dim:", encoder_last_dense_dim)
        for name, p in self.named_parameters():
            num = p.numel()
            if num > 1e6:
                print("LARGE PARAM:", name, p.shape, num)
            total += num
        print("FINAL_DENSE SHAPE:", self.final_dense.weight.shape, "params:", self.final_dense.weight.numel())
        print("TOTAL PARAMS DECODER:", total)
    ...
    def forward(self, z):
        ...
        #         self.hidden_layer_sizes[-1],
        #         self.encoder_last_dense_dim // self.hidden_layer_sizes[-1])
        # # now x is [B, C, T] ready for ConvTranspose1d
