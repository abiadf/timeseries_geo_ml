...
class ResidualConnection(nn.Module):
    ...
    def forward(self, z):
        ...
        # for deconv in self.deconv_layers[:-1]:
            x = F.relu(deconv(x))
            # print(f"[ResidualConnection] after deconv {i}: {x.shape}")  # log
        x = F.relu(self.deconv_layers[-1](x))
        # print(f"[ResidualConnection] after last deconv: {x.shape}")  # log
        x = x.flatten(1)
        # print(f"[ResidualConnection] after flatten: {x.shape}")  # log
        x = self.final_dense(x)
        # print(f"[ResidualConnection] after final_dense: {x.shape}")  # log
        residuals = x.view(-1, self.seq_len, self.feat_dim)
        # print(f"[ResidualConnection] output residuals: {residuals.shape}")  # log
        return residuals
...
class TimeVAEEncoder(nn.Module):
    def __init__(self, seq_len, feat_dim, hidden_layer_sizes, latent_dim):
        ...
        self.layers.append(nn.Conv1d(feat_dim, hidden_layer_sizes[0], kernel_size=4, stride=4, padding=2))
        ...
        for i, num_filters in enumerate(hidden_layer_sizes[1:]):
            ...
            self.layers.append(nn.Conv1d(hidden_layer_sizes[i], num_filters, kernel_size=4, stride=4, padding=2))
            self.layers.append(nn.ReLU())
