    ...
    def auto_fold_timeseries(X: np.ndarray, y: Optional[np.ndarray] = None, denoise: bool = True,
        ...
        if window_size is None:
            ...
            if window_size > max_possible_window:
                print(f"⚠️ Provided window_size {window_size} exceeds sequence length {max_possible_window}, using {max_possible_window}")
                window_size = max_possible_window

        X_folded, y_folded = WindowFolder._fold_and_stack_fixed_pages(X, window_size, y, num_pages_to_use=num_pages_to_use)
